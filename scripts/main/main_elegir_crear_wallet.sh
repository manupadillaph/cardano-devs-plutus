...
walletName=""
until [[ -f "$FALCON_DEVS_HASKELL_FILES/wallets/${walletName}.pkh" && -f "$FALCON_DEVS_HASKELL_FILES/wallets/${walletName}.skey"  && -f "$FALCON_DEVS_HASKELL_FILES/wallets/${walletName}.vkey"  ]]
do
    printf "\nNombre Wallet (01): "
    read walletName
    ...
    if ! [[ -f "$FALCON_DEVS_HASKELL_FILES/wallets/${walletName}.pkh" && -f "$FALCON_DEVS_HASKELL_FILES/wallets/${walletName}.skey"  && -f "$FALCON_DEVS_HASKELL_FILES/wallets/${walletName}.vkey"  ]]
    then
        printf "\nWallet pkh, skey, vkey files no existen\n"

        if [[ -f "$FALCON_DEVS_HASKELL_FILES/wallets/${walletName}.json"   ]]
        then
            ...
            if [[ $opcion = "y" ]]; then 
                ...
                bash $FALCON_DEVS_SCRIPTS/tools/wallet-create-con-cardano-wallet-desde-json.sh ${walletName}
        ...
        if [[ $walletLoadedInServerWallet = 0 ]]; then 
            ...
            if [[ $opcion = "y" ]]; then 
                ...
                bash $FALCON_DEVS_SCRIPTS/tools/wallet-create-con-cardano-wallet.sh ${walletName} ${walletName}
...
if [[ $walletLoadedInServerWallet = 0 ]]; then
    ...
    if [[ -f "$FALCON_DEVS_HASKELL_FILES/wallets/${walletName}.json"  ]]
    then
        bash $FALCON_DEVS_SCRIPTS/tools/wallet-load-con-cardano-wallet-desde-json.sh ${walletName}

        if ! [[ -f "$FALCON_DEVS_HASKELL_FILES/wallets/${walletName}.addrs" ]];
        then

            if  [[ -f "$FALCON_DEVS_HASKELL_FILES/wallets/${walletName}.id"  ]];
            then
                ...
                echo $DIRECCIONES | jq -r '.id' >$FALCON_DEVS_HASKELL_FILES/wallets/${walletName}.addrs
    ...
    if ! [[ -f "$FALCON_DEVS_HASKELL_FILES/wallets/${walletName}.addr" ]]
    then
...
walletSig=$(cat $FALCON_DEVS_HASKELL_FILES/wallets/${walletName}.pkh)

echo "Payment Key HASH:" $walletSig

if [[ -f "$FALCON_DEVS_HASKELL_FILES/wallets/${walletName}.json"  ]]
then
    walletPassphrase=$(cat $FALCON_DEVS_HASKELL_FILES/wallets/${walletName}.json | jq -r '.passphrase')
