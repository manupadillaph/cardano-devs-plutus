...
scriptName=""
until [[ -f "files/mintingpolicies/NFT-${scriptName}.plutus"  ]]
do
    ...
    if ! [[ -f "files/mintingpolicies/NFT-${scriptName}.plutus" ]]
    then
        printf "\nMinting Policiy file NFT-${scriptName}.plutus no existe\n"
    fi
    ...
    if [[ $opcion = "y" ]]; then 
        ...
        printf "%s\n%s\n%s\n" "16" "files/mintingpolicies" "NFT-${scriptName}" "$oref" "$tn" | cabal exec deploy-smart-contracts-auto-exe

    fi

done

policyFile="files/mintingpolicies/NFT-${scriptName}.plutus"
