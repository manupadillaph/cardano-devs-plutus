...
scriptName=""
until [[ -f "files/mintingpolicies/Signed-${scriptName}.plutus"  ]]
do
    ...
    if ! [[ -f "files/mintingpolicies/Signed-${scriptName}.plutus" ]]
    then
        printf "\nMinting Policiy file Signed-${scriptName}.plutus no existe\n"
    fi
    ...
    if [[ $opcion = "y" ]]; then 
        ...
        #printf "%s\n%s\n%s\n" "18" "files/mintingpolicies" "Signed-${scriptName}" "$walletAddr" | cabal exec deploy-smart-contracts-auto-exe
        printf "%s\n%s\n%s\n" "19" "files/mintingpolicies" "Signed-${scriptName}" "$pkh" | cabal exec deploy-smart-contracts-auto-exe

    fi

done

policyFile="files/mintingpolicies/Signed-${scriptName}.plutus"
