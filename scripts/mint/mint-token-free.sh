...
scriptPolicyName=""
until [[ -f "$FALCON_DEVS_HASKELL_FILES/mintingpolicies/Free-${scriptPolicyName}.plutus"  ]]
do
    ...
    if ! [[ -f "$FALCON_DEVS_HASKELL_FILES/mintingpolicies/Free-${scriptPolicyName}.plutus" ]]
    then
    ...
    if [[ $opcion = "y" ]]; then 
        ...
        printf "%s\n%s\n%s\n" "15" "$FALCON_DEVS_HASKELL_FILES/mintingpolicies" "Free-${scriptPolicyName}" | cabal exec deploy-smart-contracts-auto
...
policyFile="$FALCON_DEVS_HASKELL_FILES/mintingpolicies/Free-${scriptPolicyName}.plutus"
