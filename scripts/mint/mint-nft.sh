...
scriptPolicyName=""
until [[ -f "$FALCON_DEVS_HASKELL_FILES/mintingpolicies/V1/NFT-${scriptPolicyName}.plutus"  ]]
do
    ...
    if ! [[ -f "$FALCON_DEVS_HASKELL_FILES/mintingpolicies/V1/NFT-${scriptPolicyName}.plutus" ]]
    then
    ...
    if [[ $opcion = "y" ]]; then 
        ...
        printf "%s\n%s\n%s\n" "16" "$FALCON_DEVS_HASKELL_FILES/mintingpolicies/V1" "NFT-${scriptPolicyName}" "$walletTxIn" "$token_name" | cabal exec deploy-smart-contracts-auto
...
policyFile="$FALCON_DEVS_HASKELL_FILES/mintingpolicies/V1/NFT-${scriptPolicyName}.plutus"
