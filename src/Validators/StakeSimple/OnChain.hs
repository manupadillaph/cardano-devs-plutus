...
validateMasterFundPool pParams dPoolStateFromInputBeingValidated redeemerPoolNFT redeemerPoolNFTTokenName redeemerPoolNFTTxOutRef redeemerMaster redeemerFund  ctx  =
    ...
    traceIfFalse "Validate Master Fund Pool Message: Wrong NFT Minting Policy " (correctMintigPolicyUsedForNFT pParams redeemerPoolNFT  ) &&
    ...
    traceIfFalse "Validate Master Fund Pool Message: Redeemer Pool NFT not found in Own's Single Output Value" (isPoolNFTInSingleOwnOutputValue redeemerPoolNFT ctx) &&
    -- Check if the redeemerPoolNFT is the same that identifies the Pool in the PoolParams.
    traceIfFalse "Validate Master Fund Pool Message: Pool Param's NFT dosen't matches Redeemer Pool NFT" (isPoolNFTParam pParams redeemerPoolNFT ) &&
    ...
    traceIfFalse "Validate Master Fund Pool Message: Redeemer Master's signature missing" (signedByMaster redeemerMaster info) &&
    ...
    traceIfFalse "Validate Master Fund Pool Message: Output PoolState Datum wrong" (correctSingleOwnOutputPoolStateDatumWithNewFund redeemerPoolNFT redeemerMaster redeemerFund  ctx)   &&
    ...
    traceIfFalse "Validate Master Fund Pool Message: Output Value Error, Wrong sum of Input Values" (correctSumInputValuesInSingleOwnOutputValue redeemerFund ctx) &&
    ...
    traceIfFalse "Validate Master Fund Pool Message: Output Value Error, Wrong sum of PoolState Datums" (correctSumInputPoolStateDatumsInSingleOwnOutputValue redeemerFund  ctx) && 
...
validateUserInvest pParams dPoolStateFromInputBeingValidated redeemerPoolNFT redeemerUserNFT redeemerUserNFTTokenName redeemerUserNFTTxOutRef redeemerUser redeemerInvest  redeemerCreatedAt redeemerDeadline ctx  =
    ...
    traceIfFalse "Validate User Invest Message: Redeemer TxOutRef used for NFT not found in Inputs" (hasUTxO info redeemerUserNFTTxOutRef ) &&
    -- Check if the redeemerUserNFT was minted with the right minting policy
    traceIfFalse "Validate User Invest Message: Wrong NFT Minting Policy " (correctMintigPolicyUsedForNFT pParams redeemerUserNFT  ) &&
    ...
    traceIfFalse "Validate User Invest Message: User NFT is not valid. Wrong Minted quantity, must be equal 1" (isMintedNFTValid info redeemerUserNFT ) &&
    ...
    traceIfFalse "Validate User Invest Message: Redeemer User NFT not found in User's Output Value" (isUserNFTInUserOutputValue redeemerUserNFT redeemerUser info)  &&
    
    -- Check if this tx was signed by the User specified in the redeemerUser.
    traceIfFalse "Validate User Invest Message: Redeemer User's signature missing" (signedByUser redeemerUser info) &&
    ...
    -- Inside is going to fail if the outputs at the script aren't just two, one with the PoolState Datum and the other with the UserStateDatum.
    traceIfFalse "Validate User Invest Message: Output PoolState Datum wrong" (correctDoubleOwnOutputsPoolStateDatumWithNewUser redeemerPoolNFT redeemerUserNFT ctx)   &&
    ...
    -- Inside is going to fail if the outputs at the script aren't just two, one with the PoolState Datum and the other with the UserStateDatum.
    traceIfFalse "Validate User Invest Message: Output UserState Datum wrong" (correctDoubleOwnOutputsUserStateDatumOfNewUser redeemerUser redeemerUserNFT redeemerInvest redeemerCreatedAt redeemerDeadline ctx)   &&
    ...
    traceIfFalse "Validate User Invest Message: UserState's Output Value Error. Must be the same that redeemerInvest." (correctDoubleOwnOutputUserStateValue redeemerInvest ctx)
