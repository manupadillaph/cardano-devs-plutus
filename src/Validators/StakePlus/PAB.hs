...
module Validators.StakePlus.PAB
    ( 
        ValidatorContracts (..)
    ) 
    where
...
--Import Internos
import  Validators.StakePlus.OffChain   
...
data ValidatorContracts = 
    ...
    MasterFundAndMergePool MasterFundAndMergePoolParams |
...
instance HasDefinitions ValidatorContracts where
    ...
    getDefinitions        = [
            ...
            MasterFundAndMergePool exampleMasterFundAndMergePoolParams ,
    ...
    getContract (MasterFundAndMergePool mfampParams) =   SomeBuiltin $ masterFundAndMergePool @() @Empty mfampParams
